...
class SACAgent:
    def __init__(self,
        ...
        self.action_bounds = tuple(bound.to(self.device) for bound in self.config["action_bounds"])
        ...
        #                                     n_hidden_filters=self.config["hidden_sizes"][0]).to(self.device)
        self.policy_network = GaussianPolicy(self.n_states + self.n_skills,
    ...
    def train(self):
        if len(self.memory) < self.batch_size:
            ...
            # print(reparam_actions.shape, log_probs.shape)
